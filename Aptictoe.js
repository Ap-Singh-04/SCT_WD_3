...
TicTacToe.prototype.paint = function (grid_size) {
  ...
  for (let i = 0; i < grid_size; i++) {
    html += "<tr>";
    for (let j = 0; j < grid_size; j++) html += "<td></td>";
    html += "</tr>";
  }
  html += "</table>";
  ...
  for (let cell of this.columns) {
    cell.addEventListener("click", (e) => this.mark(e.target));
  }
};

TicTacToe.prototype.mark = function (cell) {
  if (cell.innerHTML) return;

  this.marks.count++;
  let mark = this.marks.count % 2 ? this.marks.X : this.marks.O;
  cell.innerHTML = mark;
  cell.classList.add(mark);

  if (this.didWin(mark)) {
    this.scores[mark]++;
    this.callback(mark, this.scores);
  } else if (this.marks.count === this.columns.length) {
    this.callback("draw");
  }
};
...
TicTacToe.prototype.didWin = function (mark) {
  let size = this.grid_size, rToL = 0, lToR = 0;

  for (let i = 0; i < size; i++) {
    let row = 0, col = 0;
    for (let j = 0; j < size; j++) {
      if (this.columns[i * size + j].innerHTML === mark) row++;
      if (this.columns[j * size + i].innerHTML === mark) col++;
    }
    if (row === size || col === size) return true;
    if (this.columns[i * size + i].innerHTML === mark) rToL++;
    if (this.columns[(size - 1) * (i + 1)].innerHTML === mark) lToR++;
  }
  return rToL === size || lToR === size;
};

TicTacToe.prototype.empty = function () {
  for (let cell of this.columns) {
    cell.innerHTML = "";
    cell.classList.remove(this.marks.X, this.marks.O);
  }
  this.marks.count = 0;
};
...
const winPopupContent = document.getElementById("win-popup-content");
...
function pulseBackgroundText() {
  document.body.classList.add("pulse-bg");
  setTimeout(() => document.body.classList.remove("pulse-bg"), 1000);
}
...
function updateScores(X, O) {
  ...
  document.getElementById("player2").textContent = O;
}

// Reset button — resets everything including scores
function restart() {
  tictactoe.resetScores();
  updateScores(0, 0);
  tictactoe.paint(tictactoe.grid_size);
}

// Dropdown — changes grid size but keeps scores
function changeGridSize(size) {
  tictactoe.empty();
  tictactoe.paint(parseInt(size));
}
